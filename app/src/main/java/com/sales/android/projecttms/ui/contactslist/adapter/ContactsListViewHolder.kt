    ...
    fun bind(contact: ContactData) {
        binding.run {
            ...
            if (contact.dateOfNextContact == convertLongToTime(Date().time)) {
                itemView.setBackgroundColor(Color.parseColor("#13803e"))
            }
            if (contact.statusOfContact != StatusOfContact.REFUSE.status && convertTimeToLong(
                    contact.dateOfNextContact
                ) <
                    convertTimeToLong(
